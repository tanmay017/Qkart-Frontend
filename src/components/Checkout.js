...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    let errored = false;
    try {
      const response = await fetch(`${config.endpoint}/user/addresses/${addressId}`, {
        method : "DELETE",
        headers: {
          Authorization : `Bearer ${localStorage.getItem('token')}`
        }
    ...
    if(this.validateResponse(errored, data, "Delete Address")) {
      await this.getAddresses();
    }
  };
  ...
  order = async () => {
    ...
    const selectedIndex = this.state.selectedAddressIndex;
    console.log(selectedIndex);
    console.log(addresses.length);
    if(balance < totalCost) {
      message.error("Your wallet balance is less than total cost");
    }
    else if(addresses.length === 0) {
      message.error("Please select or add an available address");
    }
    else {
      await this.checkout();
    }
  ...
  async componentDidMount() {
    ...
    const cartProducts = await this.cartRef.current.getCart();
